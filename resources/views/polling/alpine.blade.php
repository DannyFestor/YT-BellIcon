    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('Notifications', () => ({
                ...
                updateNotifications() {
                    axios.get('{{ route('polling.alpine.show') }}')
                        .then(res => {
                            if (res.data.public && res.data.public.length > 0) {
                                this.publicNotifications = res.data.public;
                            } else {
                                this.publicNotifications = [];
                            }

                            if (res.data.private && res.data.private.length > 0) {
                                this.privateNotifications = res.data.private;
                            } else {
                ...
                init() {
                    setInterval(() => this.updateNotifications(), 1000);
                },
            }));
        });
    </script>
